...
class PLUS_DM(IndicatorUtils):
    def analyze(self, historical_data, period_count=14,
                signal=['plus_dm'], hot_thresh=None, cold_thresh=None):
        
        dataframe = self.convert_to_dataframe(historical_data)
        dmi_values = abstract.PLUS_DM(dataframe).to_frame()
        dmi_values.dropna(how='all', inplace=True)
        dmi_values.rename(columns={0: 'plus_dm'}, inplace=True)
